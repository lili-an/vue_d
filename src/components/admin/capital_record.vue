<template>
  <div class="investment_record">
    <mt-header title="资金记录" fixed class="w-header">
      ...
      loadMore() {
        ...
        setTimeout(() => {
          ...
          var projectObj = {
              ...
              api.BillList(projectObj).then(res => {
                if(res.errMsg == 'SUCCESS') {
                  this.loadinging = false

                  for(let i=0; i<res.result.length; i++) {
                    setTimeout(function() {
                      if(res.result[i].income == 1) {
                        document.getElementById('b'+res.result[i].id).style.color = '#1c71f5'
                        document.getElementById('s'+res.result[i].id).innerHTML = '-'
                      }
                    }, 500)
                  }
