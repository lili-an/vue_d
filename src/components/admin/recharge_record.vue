<template>
  <div class="investment_record">
    <mt-header title="充值记录" fixed class="w-header">
      ...
      loadMore() {
		  ...
		  setTimeout(() => {
		    ...
		    var projectObj = {
		        ...
		        api.RechargeList(projectObj).then(res => {
		          if(res.errMsg == 'SUCCESS') {
		          	this.loadinging = false

		          	for(let i=0; i<res.result.length; i++) {
		              if(res.result[i].status == 0) {
		                  res.result[i].status = '申请中'
		              }else if(res.result[i].status == 1) {
		                  res.result[i].status = '申请成功'
		              }else if(res.result[i].status == 2) {
		                  res.result[i].status = '申请失败'
		              }
		            }
